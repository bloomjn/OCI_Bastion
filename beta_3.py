...
def commands(type, ip_addr, remote_port):
    ...
    if type == "SOCKS5":
        ...
        cmd = "ssh", "-o", "serveraliveinterval=60", "-o", "StrictHostKeyChecking no", "-N", "-D", "{}:{}".format("127.0.0.1", local_port), "{}@{}".format(bastion_session_ocid, bastion_fqdn)
    if type == "LOCAL":
        print("{}:{} <--MAPPED TO--> localhost:{}".format(ip_addr, remote_port, local_port))
        cmd = ("ssh", "-o", "serveraliveinterval=60", "-o", "StrictHostKeyChecking no", "-N", "-L", "{}:{}:{}".format(local_port, ip_addr, remote_port), "{}@{}".format(bastion_session_ocid, bastion_fqdn))
    return cmd
async def run_cmd():
    global cmd
    try:
